...
simulate_cov_cognate <- function(tree, Q) {
  ...
  root_node <- n_taxa + 1
  states[root_node] <- sample(1:4, 1)  # Randomly select F0, F1, S0, or S1
...
write.tree(tree, file="../simulated_data/simulated_tree.nwk")
...
write.table(latitude_df, "../simulated-data/simulated_latitudes.dat", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)


longitude_df <- data.frame(Language = tree$tip.label,
                           Longitude = tip_coords[, 1])
write.table(longitude_df, "../simulated-data/simulated_longitudes.dat", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
